---
description: 旧版实施使用 Dynamic Tag Management (DTM) 来设置和部署 Experience Cloud ID 服务，并将其与您的其他 Experience Cloud 解决方案相集成。
keywords: ID 服务
seo-description: 旧版实施使用 Dynamic Tag Management (DTM) 来设置和部署 Experience Cloud ID 服务，并将其与您的其他 Experience Cloud 解决方案相集成。
...
旧版实施使用 Dynamic Tag Management (DTM) 来设置和部署 Experience Cloud ID 服务，并将其与您的其他 Experience Cloud 解决方案相集成。
...
在尝试使用 Dynamic Tag Management (DTM) 实施 Experience Cloud ID 服务之前，请查看这些要求和程序。
...
客户必须提供 CNAME 或使用 [!DNL *.sc.omtrdc] 进行[地区数据收集](https://marketing.adobe.com/resources/help/zh_CN/whitepapers/rdc/) (RDC)。请从您的 [!DNL Adobe] 顾问处获取具体的 RDC 设置。
...
1. 在 **[!UICONTROL 工具类型]** 列表中，单击 **[!UICONTROL Experience Cloud ID 服务]**。
...
选中此选项后，Dynamic Tag Management 会先自动调用 `getMarketingCloudVisitorID()` 方法，然后再加载任何使用 Experience Cloud ID 服务的 Adobe 解决方案。
...
集成代码是客户提供的唯一 ID。集成代码应包含用于在 [!DNL Audience Manager] 中[创建数据源](https://marketing.adobe.com/resources/help/zh_CN/aam/create-datasource.html)的值。
...
## 测试和验证 Experience Cloud ID 服务 {#concept-644fdbef433b46ba9c0634ac95eaa680}
...
当您在 [!DNL Adobe] 调试器响应中看到 [!DNL Experience Cloud ID] (MID) 时，即表明您的服务集成配置正确。请参阅 [Cookie 和 Experience Cloud ID 服务](../introduction/cookies.md)，以了解有关 MID 的更多信息。
...
当[数据收集服务器](https://marketing.adobe.com/resources/help/zh_CN/aam/c_compcollect.html) (DCS) 响应返回 MID 时，即表示您的帐户已针对 ID 服务进行正确配置。MID 作为使用以下语法的键值对返回：`d_mid: *`访客 Experience Cloud ID`*`。在 [!DNL Response] 选项卡中查找 MID，如下所示。
...
有关错误代码的更多信息，请参阅 [DCS 错误代码、消息和示例](https://marketing.adobe.com/resources/help/zh_CN/aam/dcs_error_codes.html)。
