---
description: 这些说明适用于那些希望使用 Experience Cloud ID 服务而不使用动态标签管理 (DTM) 的 Analytics、Audience Manager 和 Target 客户。但是，我们强烈建议您使用 DTM 来实施此 ID 服务。DTM 可简化实施工作流程，并自动确保代码放置和排序的正确性。
keywords: ID 服务
seo-description: 这些说明适用于那些希望使用 Experience Cloud ID 服务而不使用动态标签管理 (DTM) 的 Analytics、Audience Manager 和 Target 客户。但是，我们强烈建议您使用 DTM 来实施此 ID 服务。DTM 可简化实施工作流程，并自动确保代码放置和排序的正确性。
...
这些说明适用于那些希望使用 Experience Cloud ID 服务而不使用动态标签管理 (DTM) 的 Analytics、Audience Manager 和 Target 客户。但是，我们强烈建议您使用 DTM 来实施此 ID 服务。DTM 可简化实施工作流程，并自动确保代码放置和排序的正确性。

>[!IMPORTANT]
>
>在开始之前请阅读ID服务 [要求](../mcvid-reference/mcvid-requirements.md) ，并请注意特定于此实施的以下要求：&gt;
...
如果您不确定如何查找跟踪服务器，请参阅[常见问题解答](../mcvid-faq-intro/ecid-faq.md)以及[正确填充 trackingServer 和 trackingServerSecure 变量](https://helpx.adobe.com/analytics/kb/determining-data-center.html#)。
...
* [托管页面的域中的 AMCV Cookie](../mcvid-introduction/mcvid-cookies.md)。
* [!DNL Experience Cloud] ID(MID)与 [!DNL Adobe] 调试器或首选调试工具。

有关帮助您确定ID服务是否正常运行的其他检查，请参阅 [测试和验证Experience Cloud ID服务](../mcvid-implementation-guides/mcvid-test-verify.md)。
...
* 您启用 [了宽限期](../mcvid-reference/mcvid-analytics-reference/mcvid-grace-period.md) 。
...
* 当您符合[中止宽限期的条件](../mcvid-implementation-guides/mcvid-setup-aam-analytics-target.md#section-aceacdb7d5794f25ac6ff46f82e148e1)时，请记得禁用宽限期。
