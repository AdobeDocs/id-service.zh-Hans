---
description: 通过这些配置，在iFrame中和父页面上实施的访客ID服务代码的不同实例可以相互通信。 它们旨在帮助解决以下两种特定用例中出现的问题：您可能会（也可能不会）控制父页面/域，以及您在自己控制的域的iFrame中加载访客ID服务代码。 它们在 VisitorAPI.js 代码版本 2.2 或更高版本中可用。
keywords: 访客 ID 服务
...
通过这些配置，在iFrame中和父页面上实施的访客ID服务代码的不同实例可以相互通信。 它们旨在帮助解决以下两种特定用例中出现的问题：您可能会（也可能不会）控制父页面/域，以及您在自己控制的域的iFrame中加载访客ID服务代码。 它们在`VisitorAPI.js`代码版本2.2或更高版本中可用。
...
## 句法 {#section-f645198bbaba4fba8961acb6e88d1470}
...
您配置的 [!UICONTROL Visitor ID Service] 代码可能与以下示例类似。
...
这些配置有助于解决在浏览器阻止第三方Cookie并且满足以下任一条件时，设置访客ID服务Cookie和分配访客ID的问题：

* 您控制或不控制父页面/域。
* 访客ID服务代码未安装在父页面上，而是在iFrame中实施。

>[!TIP]
>
>在 iFrame 中使用[视频心率](https://experienceleague.adobe.com/docs/media-analytics/using/media-overview.html?lang=zh-Hans)为视频提供服务时，您可能还希望实施这些配置。 视频心率需要访客ID服务ID (MID)才能正常工作。

**用例1：浏览器阻止第三方Cookie，并在iFrame和父页面上实施访客ID服务**
  ...
  <tr> 
   ...
   <td colname="col2"> <p>此用例包括以下条件： </p> <p> 
     <ul id="ul_DC748846585745B0AB74398D82BDA53A"> 
      <li id="li_6E04CF0B6A204B4D8856656B0C9EF2A5">公司A在其主页上实施访客ID服务。 </li> 
      <li id="li_B53AE0F0C69844E7B6C4D3464C57883B">公司A在其主页上的iFrame中实施访客ID服务。 </li> 
      <li id="li_07E0A6D7BEB140E4B9FB6C7B9629B860">公司A拥有父页面和iFrame，并已在这两个位置实施了访客ID服务。 </li> 
      <li id="li_76967BD69DDB40A8A9C915DADC58AC62">客户在浏览器中加载父页面，该浏览器会阻止第三方 Cookie。 </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>结果</b> </p> </td> 
   <td colname="col2"> <p>在这些条件下，访客ID服务： </p> <p> 
     <ul id="ul_12356701501E40DFA57903494FFE58F7"> 
      <li id="li_B57EDF1B0762486F95FA6526C047390C">在父页面上可正常运行。 它会请求和设置 AMCV Cookie，并为网站访客分配唯一 ID。 </li> 
      <li id="li_BA9F42C759E747EAAE14DD3FBB6130A5">在 iFrame 中无法运行。 这是因为浏览器将iFrame视为第三方域并阻止访客ID服务设置AMCV Cookie。 </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>解决方案</b> </p> </td> 
   <td colname="col2"> <p>使用这些白名单配置修改iFrame中的访客ID服务<span class="codeph"> Visitor.getInstance </span>函数。 在代码中指定父域和子域。 通过这些配置，iFrame中的访客ID服务代码可在父页面上的访客ID服务代码中检查访客ID。 </p> <p>如果iFrame中的访客ID服务代码未收到响应父页面，则这些配置将生成本地访客ID。 </p> </td> 
  </tr> 
 </tbody> 
</table>

**用例2：从不由您控制或不使用访客ID服务的父页面中嵌入的iFrame请求ID**
  ...
  <tr> 
   ...
   <td colname="col2"> <p>此用例包括以下条件： </p> <p> 
     <ul id="ul_356E8FB0B1D14F46A844FE5281967E28"> 
      <li id="li_1285D945361842268B46FB492A3B5AA5">公司A不使用访客ID服务。 </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在这些条件下，访客ID服务： </p> <p> 
     <ul id="ul_A92D90896E5A42C5804AC5CE83E8EB25"> 
      <li id="li_9734EA9C5D9D4F908DE783188C9E5530">在 iFrame 中无法运行。 这是因为浏览器将iFrame视为第三方域并阻止访客ID服务设置AMCV Cookie。 </li> 
...
* 在父域和iFrame域上实施的访客ID服务必须使用相同的IMS组织ID。 当父项或iFrame中的IMS组织ID不同时，这些白名单配置将不起作用。
* 这些配置仅与代码中指定的域和 iFrame 通信。
* iFrame 与父页面之间的通信遵循特定格式。 如果父页面上的访客ID服务未收到采用预期格式的请求，则此共享过程将失败。

## 支持的访客API方法 {#section-30c6a9f4dcdc4265a1149260b97cc057}

在实施这些白名单配置时，访客ID服务支持一组有限的公共API方法。 支持的方法因上文所述的用例情景而异。
