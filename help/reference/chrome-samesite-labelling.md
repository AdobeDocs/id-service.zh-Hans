...
2020 年 2 月，Google 发布了 Chrome 80，在该版本中，如果 Cookie 不具备指定的 SameSite 属性值，那么默认设置将由 `none` 更改为 `lax`。 该设置可防止在第三方环境（也称为“跨站点”）中使用 Cookie。 任何后续的第三方 Cookie 都必须设置为 `SameSite=none` 并标为安全。
