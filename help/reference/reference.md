---
description: 关于其他 Experience Cloud Identity 服务特性和功能的信息。
keywords: ID 服务
seo-description: 关于其他 Experience Cloud Identity 服务特性和功能的信息。
seo-title: 参考
title: 参考
uuid: 9e8f50db-1fdf-4fbc-89e5-4431bb6402d5
translation-type: tm+mt
source-git-commit: e6d65f1bfed187d7440512e8f3c2de0550506c95

---


# 参考 {#reference}

关于其他 Experience Cloud Identity 服务特性和功能的信息。

* [Analytics 引用概述](analytics-reference/analytics-reference.md)
* [内容安全策略和 Experience Cloud Identity 服务](csp.md)
* [Experience Cloud Identity 服务中的 COPPA 支持](coppa.md)
* [Experience Cloud Identity 服务中的 CORS 支持](cors.md)
* [客户 ID 和身份验证状态](authenticated-state.md)
* [从 AMCV Cookie 或 ID 服务获取区域 ID 和用户 ID](regions.md)
* [Experience Cloud Identity 服务的要求](requirements.md)
* [视频心率和 Experience Cloud Identity 服务](heartbeat.md)
* [Data Workbench 和 Experience Cloud Identity 服务](dwb.md)

