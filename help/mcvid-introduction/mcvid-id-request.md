...
# Experience Cloud ID服务请求的请求和设置ID{#how-the-experience-cloud-id-service-requests-and-sets-ids}

概述 ID 请求和响应过程。下面的示例涵盖了各种网站类型中的 ID 分配情况：单独的网站上、不同的网站之间，以及由 Experience Cloud 不同客户使用其各自组织 ID 管理的网站。

>[!NOTE]
>
>如果您不熟悉Experience Cloud ID服务如何创建访客ID，请花一点时间查看 [Experience Cloud](../mcvid-introduction/mcvid-cookies.md)。
